...
var eventPageCnt = 10;
var eventTotalCnt = 0;
var lvar_param_prefix  = "event_scan_completion_detail"; 
...
$(function () {
	...
	if(selectedScanStatus) searchKeywordList.push("scan_status|"+selectedScanStatus); // 최초 진입 시 scanStatus 상태 전달. 없는경우 totals 조회
...
/**
 * 스캔 결과에 대한 데이터 구분(페이지 진입시 최초에만 가져오도록 함)
 */
...
function scanCountDetail(data){	
	...
	$("#scanningCnt").html(scanningCnt);
	$("#scanCompletedCnt").html(scanCompletedCnt);
	$("#watingScanCnt").html(watingScanCnt);
	$("#notScanCnt").html(totalCnt-scanningCnt-scanCompletedCnt-watingScanCnt);
}
...
function searchBtnClick(selectedStatus){
	...
	if(!searchKeyword){
		...
		searchKeywordList.push("message||image_tag|"+$("#searchKeyword").val());
	}
