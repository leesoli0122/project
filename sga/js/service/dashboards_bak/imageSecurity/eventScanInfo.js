...
var eventPageCnt = 10;
var eventTotalCnt = 0;
var lvar_param_prefix  = "event_scan_detail"; // 중요. TODO topic 없애고 해당 데이터로 변경
...
$(function () {
	...
	if(selectedScanStatus) searchKeywordList.push("scan_status|"+selectedScanStatus); // 최초 진입 시 scanStatus 상태 전달 (null인 경우 전체가 조회됨)
	createScanResult();
...
function scanCountDetail(data){	
	...
	$("#successedCnt").html(scanSuccessCnt);
	$("#failedCnt").html(scanFailCnt);
	$("#etcCnt").html(totalCnt-scanSuccessCnt-scanFailCnt);
}	
 ...
 */
function cntClick(selectedStatus){
	if(selectedStatus) selectedScanStatus = selectedStatus; // 이미지 스캔 상태값 저장
	/*
	searchKeywordList = []; // 검색 키워드 배열 초기화
	if(selectedScanStatus && selectedScanStatus != 'totals'){ // 스캔 상태 필터링 조건에 추가 + totals는 필터링 검색 x
...
function searchBtnClick(selectedStatus){
	...
	if(!searchKeyword){
		...
		//searchKeywordList.push("message|"+$("#searchKeyword").val());
		//searchKeywordList.push("repository|"+$("#searchKeyword").val());
		//searchKeywordList.push("image_tag|"+$("#searchKeyword").val());
		searchKeywordList.push("message||repository||image_tag|"+$("#searchKeyword").val());
	}
...
function lf_serviceCall600401(pageNum){
	...
	if(!pageNum && pageNum != 0){
		...
		case 'last': pageNum = Math.ceil(eventPageCnt/eventPageCnt)-1; break;
		}
	}
...
function createResultPage(data,body){
	...
	$.each(dataList, function(idx, rowData){
		var tableRow = table.row.add({
			...
			"Status": rowData['status'] ? imageStatusList[rowData['status']] : '-',
