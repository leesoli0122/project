...
var lvar_param_prefix = 'event_fileCtl';
var lvar_param_trId = '700006';

var lvar_eventfileCtl_searchFieldStr = {
	'starttime': '시작일시',
	'endtime': '종료일시',
	'path': '파일 경로',
	'message': '이벤트 내용',
...
var lvar_eventfileCtlOperation = {
	1: 'equals(=)',
	11: 'not equals(!=)',
	2: 'like',
	12: 'not like',
...
function lf_addEventfileCtlConditionValue(){
	...
	if(type == 'searchtime'){
		...
		if(!value) {
			swal(lvar_eventfileCtl_searchFieldStr[type] + " 값이 비어있습니다.", {
		        icon: "./assets/images/icon_alert04.png",
		        buttons:"확인"
		    });
			return;
	...
	if(type == 'searchtime' && queryTable.find("span:contains('" + lvar_eventfileCtl_searchFieldStr['searchtime'] + "')").length > 0) {
	...
	if(type == 'equiplist'){
		...
		$TR.append($('<p><span>'+lvar_eventfileCtl_searchFieldStr[type]+'</span> / <span>'+value+'</span></p>'));	
	}else {
		$TR.append($('<p><span>'+lvar_eventfileCtl_searchFieldStr[type]+'</span> / <span>'+lvar_eventfileCtlOperation[operation]+'</span> / <span>'+value+'</span></p>'));
	}
	
	$TR.append($('<a class="removex" title="삭제" onclick="javascript: $(this).parent().remove();"></a>'));
	queryTable.find('ul').append($TR);
	
	lf_eventfileCtlsearchClear();
}

function lf_eventfileCtlsearchClear(){
	$('.fileCtl_date').val("");
...
function lf_eventfileCtlSelectType(thiz){
	var value = $(thiz).val();
	$('#event_fileCtl_search_value_field').children().hide();
	
	if(value == 'date' || value == 'equiplist'){
		$('#event_fileCtl_search_operation').hide();
		$('#event_fileCtl_search_operation_not').fadeIn();
	}
	else{
		$('#event_fileCtl_search_operation').fadeIn();
		$('#event_fileCtl_search_operation_not').hide();
	}
	
	$('#event_fileCtl_search_value_field').find('#event_fileCtl_'+value).fadeIn();
...
function lf_initEventfileCtl(){
	...
	lf_serviceCall700006();
}

function lf_eventfileCtlClick(){
...
function lf_serviceCall700006(pageNum){
	...
	cf_requestServer(_TR_EVENT_FILECTL_SEARCH,body,lf_serviceCall700006Callback);
}

function lf_serviceCall700006Callback(data, body){
	...
	$.each(dataList, function(idx, rowData){
		...
		table.row.add([
			...
			"<td class=\'long_w\'>"+rowData['path']+"</td>",
	...
	.attr({
		rel : 'eventfileCtlDetail_modal',
		onclick : 'lf_eventfileCtlTableClick(this);'
	});
	
	if($("#exportFile").length == 0) {
		$('#event_fileCtl_result_table_length').children('div:eq(0)').append(
			'<span id=exportFile data-id=appctl data-format=excel data-ui=' + lvar_param_prefix 
...
function lf_serviceButtonDataCallback(data){
	var pageArr = $('#fileCtl_paging').val().split('_');
	...
	$('#fileCtl_paging').val(pagingValue);	
}

function lf_eventfileCtlTableClick(obj){
	// 2023-09-07 이성호 페이지 넘버링 변경
	//var num = $(obj).children(':first').text();
	var num = $(obj).children(':first').children(':last').text();
	$('#detailNum').val("");
	$('#detailNum').val("fileCtl_"+num);
...
function _addPageButton(settings, api, host, idx, button, page, pages, counter){
	...
	var clickHandler = function ( e ) {
		...
		if ( !$(e.currentTarget).hasClass('disabled')) {
			lf_serviceCall700006(e.data.action);
		}
		mscrollbarReset();
	};
