...
$(function () {
	...
	if(selectedScanStatus) { // 선택 된 스캔 상태가 존재하는 경우
		...
		searchKeywordList.push("scan_status|"+selectedScanStatus); // 최초 진입 시 scanStatus 상태 전달 (null인 경우 전체가 조회됨)
	}
	createScanResult();
...
function searchBtnClick(selectedStatus){
	...
	if(selectedScanStatus && selectedScanStatus != 'total'){ // 스캔 상태도 필터링 조건에 추가 + total는 필터링 검색 x
