...
function lf_makeChart(chartType, chartId){
	...
	// 2023-12-06 jkcho 추가 > 컨테이너 이미지 스캔 Summary의 경우 contBtnBox 숨김처리
	if(chartConfData['title'] =="컨테이너 이미지 스캔 Summary"){ 
		$('.dashboard_chart_donut_box').parent().find('.cont_btn_box .sel_box').hide();
	}
...
function lf_createChartCard(chartType, paramChartConf, chartId){
	...
	if(chartElConf['NChart']){ // 2023-09-23 이성호 추가 > 1카드 N차트 > 추후 더블 이외의 N차트로 확장 가능하도록 설계
		var contGroupDIV = $('<div class="group"></div>');
	...
	contLI.append(contCommonDIV);
		...
		$.each(chartElConf['chart'],function(index,item){
			...
			contGroupDIV.append($('<div class="dashboard_'+chartElConf['chart'][index]+'_box '+chartElConf['applyCSS']+' nchart"><div id="'+chartId[index]+'" class="chart_box dashboard_'+chartElConf['chart'][index]+'"></div></div>'))	
	
			if(chartConf) contGroupDIV.find('#'+chartId[index]).data('chartConf', chartConf);
			else contGroupDIV.find('#'+chartId[index]).data('chartConf', lvar_defaultChartConf);
	
			contGroupDIV.find('#'+chartId[index]).data('chartType', chartType);	
			tabContEl.append(contLI);
		});
		contLI.append(contGroupDIV);	
	}else{
		if(chartElConf['params'].indexOf('term') >= 0){
		if(chartConf['term'] == 'DAY') {
			contBtnBox.append(
