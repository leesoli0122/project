...
function lf_getChartELConf(chartType) {
	...
	/****************************************************************************/
		...
		returnData['width'] = 50; // 01-22 디자인 변경으로 인한 크기 수정 25 -> 50
		returnData['chart'] = 'top5';
 ...
 * Table
 ******************************************************************************/
function lf_setTable(chartData, elId, popupUrl) {
	$('#' + elId).fadeOut();
	var $targetDIV = $('#' + elId);
	...
	// popupUrl을 파라미터로 받을 경우
	// modal 사용시 null 또는 파라미터 미기입
	if (popupUrl) {
		popupData = popupUrl;
	}
	...
	if (chartData['header']) {
		...
		for (var i = 0; i < chartData['header'].length; i++) {
			...
			if (chartData['header'][i] === 'Severity') severityTd = i;
		}
	...
	if (chartData['data']) {
		...
		for (var i = 0; i < chartData['data'].length; i++) {
			var tr = $('<tr onclick="javascript: lf_clkEventTable(this, \'' + chartType + '\', \'' + popupData + '\');"></tr>');
			tr.data('rowData', chartData['realData'][i]);

			for (var j = 0; j < chartData['data'][i].length; j++) {
				if (severityTd != null && severityTd === j) {
					tr.append($('<td class="long_w mw72">'+ccf_displaySeverityBCSetting(chartData['data'][i][j])+'</td>'));
				} else {
	...
	// 240124 추가 ordering option
	var orderingState = false;
	if(chartData["ordering"]){
		orderingState = true;
	}
	// 240125 추가 columnDefs option
	var columnDefsConfig = [];
	// 정렬 조건 기준에 severity 가 존재하는 경우 ,
	if(chartData["severity"]){ 
		columnDefsConfig.push({
                "targets": chartData["severity"][0], // 3번째 컬럼에 해당하는 인덱스 (0부터 시작)
                "orderData" : chartData["severity"][1]
        	});
        columnDefsConfig.push({ 
				"targets":  chartData["severity"][1], // 실제 검사하는 데이터는 숨김 처리
				"visible": false
			});
	}
	// 컬럼에서 숨기고 툴팁으로 보여줄 컬럼 번호(순서) >> 컨테이너 이벤트 에서 사용
	if(chartData["hideColumToolTip"]){
		columnDefsConfig.push({
				"targets": [chartData["hideColumToolTip"]],
				"visible": false
			});
		columnDefsConfig.push({
				"targets": chartData["hideColum"],
				"visible": false
			});
	}
	...
	if (chartData["paging"]) {
		$targetDIV.find('.tbl > table').dataTable({
			...
			"autoWidth": false,
			"paging": "full_numbers",
			"ordering": orderingState, // 상태 추가
			"order": [], // 초기 정렬 제거
			"info": false,
			"filter": false,
			"lengthChange": true,
			"dom": 't<"table-bottom"p>',
			"columnDefs": columnDefsConfig // 상태 추가
		});
	}
	else if (chartData["hideColumToolTip"]) {	// 컬럼에서 숨기고 툴팁으로 보여줄 컬럼 번호(순서) >> 컨테이너 이벤트에서만 사용.
		$targetDIV.find('.tbl > table').dataTable({
			"autoWidth": false,
			"paging": false,
			"ordering": orderingState,
			"order": [chartData["severity"][0],"asc"], // 컨테이너 이벤트는 초기 정렬 조건을 Severity 오름차순으로 설정.
			"info": false,
			"filter": false,
			"lengthChange": true,
...
function lf_clkEventTable(thiz, chartType, popupUrl) {
	var param = {};
	...
	if (popupUrl != 'null') { // popupUrl 정보가 들어왔을 경우, 해당 페이지를 오픈		
		$('#detailDataInfo').val('');
		$('#detailDataInfo').val(JSON.stringify(param['rowData']));
		window.open(popupUrl, '', 'width=1500,height=810,location=no,status=no,scrollbars=yes');
	} else {
...
function lf_setComplianceTable(elId) {
	$('#' + elId).fadeOut();
	$('#' + elId).addClass('dashboard_compliance_table');
...
function lf_serviceCall600501CallBack(data, body) {
	...
	if (dataList.length > 0) {
		var chartData = {
			...
			'realData': [],
			'ordering': true,
			'severity': [2,4], // severity 가 있는 경우, chartData 옵션 추가 
		};
		...
		for (var i = 0; i < dataList.length; i++) {
			...
			var cnt = dataList[i].cnt;
			row.push(cnt);
			row.push(makeSeverityOrderingData(severity)); // severity 정렬 조건 추가
...
function showClusterSelectedDiv(chartId) {
	var targetDiv = $("#" + chartId).parent().parent().parent().parent().parent().parent().find('.dashboard_box_cluster');
...
function lf_serviceCall600600CallBack(data, body) {
	...
	if (dataList.length > 0) {
		...
		if (data.body) {
			...
			CustomParam["param"]["clusterUuid"] = body.clusterUuid;
		}
...
function lf_serviceCall600601CallBack(data, body) {
	...
	if (dataList.length > 0) {
		...
		if (data.body) {
			if (body.is_user_scan_enable) CustomParam["param"]["is_user_scan_enable"] = body.is_user_scan_enable;
			if (body.is_scan_enable) CustomParam["param"]["is_scan_enable"] = body.is_scan_enable;
			CustomParam["param"]["cluster_uuid"] = body.clusterUuid;
		}
...
function lf_serviceCall600610(chartId, param) {
	var selectedClusterUuid = showClusterSelectedDiv(chartId);
	var termData = param.term?param.term:$('#' + chartId).parent().parent().parent().find('.sel_box select').val();
	cf_contPreloader(chartId);
	...
	cf_contPreloader(chartId);
	//$('#'+chartId).data('chartConf')['term'] = $('#'+chartId).parent().parent().find('.sel_box select').val();
	$('#' + chartId).data('chartConf')['term'] = termData;
	//600610
...
function lf_serviceCall600610CallBack(data, body) {
	...
	if (dataList.length > 0) {
		...
		var chartData = {
			...
			'colgroup': ['100', 'auto', '100', '50'],
			'data': [],
			'realData': [],
			'hideColumToolTip': 4,
			'hideColum': [5, 6, 7, 8],
			'displayTooltip': 1,
			'ordering': true, // 정렬 옵션 추가 
			'severity': [2 , 9], // severity 가 있는 경우, chartData 옵션 추가 
		};
		...
		for (var i = 0; i < dataList.length; i++) {
			...
			row.push(dataList[i].policy_id);
			row.push(body.term);
			row.push(body.clusterUuid);
			row.push(dataList[i].fixDate);
			row.push(makeSeverityOrderingData(dataList[i].severity)); // severity 정렬 조건 추가
			chartData['data'].push(row);

			dataList[i]['term'] = body.term;
			dataList[i]['clusterUuid'] = body.clusterUuid;
			chartData['realData'].push(dataList[i]);


		}
		lf_setTable(chartData, chartId, '/eventContainerEventByPolicyInfo.do');
	}
	else {
		lf_createNotDateView(chartId);
	}
...
function lf_serviceCall600612CallBack(data, body) {
	...
	if (dataList.length > 0) {
		...
		var chartData = {
			'header': ['Name', 'Severity', 'Cnt'],
			'colgroup': ['auto%', '100', '50'],
			'data': [],
			'realData': [],
			'hideColumToolTip': 3,
			'displayTooltip': 0,
			'hideColum': [4, 5, 6, 7, 8, 9],
			'ordering': true, // 정렬 옵션 추가 
			'severity': [1 , 10], // severity 가 있는 경우, chartData 옵션 추가 
		};
		...
		for (var i = 0; i < dataList.length; i++) {
			...
			dataList[i]['term'] = body.term;
			dataList[i]['clusterUuid'] = body.clusterUuid;
			dataList[i]['type'] = body.type;
			chartData['realData'].push(dataList[i]);

		}
		lf_setTable(chartData, chartId, '/eventContainerEventByRuleInfo.do');
	}
	else {
		lf_createNotDateView(chartId);
	}
...
function lf_serviceCall600613CallBack(data, body) {
	...
	var colorSet = ["#C1282E", "#F7941F", "#FCEE21"];
	var gridSet = {
		top: '30',
		left: '20',
		right: '130',
...
function lf_serviceCall600615CallBack(data, body) {
	...
	var legendSet = {
		...
		itemHeight: 15,
		inactiveColor: '#3D3D3D',
		data: ['CRITICAL', 'HIGH', 'MEDIUM', 'LOW', 'IGNORE'],
		textStyle: {
			fontFamily: 'S-CoreDream',
			fontWeight: '400',
			fontSize: 11,
	...
	var colorSet = ["#C1282E", "#F7941F", "#FCEE21", "#39B549", "#b2b2b2"];
	var gridSet = {
		top: '30',
		left: '20',
		right: '130',
...
function lf_serviceCall600621CallBack(data, body) {
	...
	if (countList != "noData") {		
		createDonutChartForContainerEvent(chartId, 'container security risk', ["#C1282E", "#F7941F", "#FCEE21", "#39B549", "#b2b2b2"], countList);
	} else {
		lf_createNotDateView(chartId);
	}
	lf_DashboardEventInit();
...
function lf_setNLineChart(dataList, chartId, groupData, timeList, title, legendSet, colorSet, gridSet) {
	...
	if (dataList) {
		...
		var lineChart = echarts.init(document.getElementById(chartId), 'dark', {
			renderer: 'canvas',
			useDirtyRect: false
		});
...
//makeSeverityOrderingData
function makeSeverityOrderingData(severity){
	var severityOrdering=0;
	// severity 에 따른 , ordering 데이터를 추가 
	switch(severity){
		case "CRITICAL":
		case "Critical":
			severityOrdering = 0;
