...
function lf_openDashboardFullWindow(){  
	var newWindow = window.open('./dashboardView.do','','height=' + screen.height + ',width=' + screen.width + 'fullscreen=yes');
	newWindow['dashboardData'] = lvar_dashboardData[lf_getActiveTabIdx()-1];
}
...
function lf_dashboardCreateChartCallBack(){
	...
	if(chartIdData.length > 0){
		...
		for(var i = 0 ; i < chartIdData.length; i++){
			var chartId = lf_createChartCard(lf_getActiveTabIdx(), chartIdData[i]);
...
function lf_saveEventFunc(){
	var saveData = [];
	//console.log(lvar_dashboardData);
...
function lf_openTabEditDialog(thiz){
	...
	var callBackFn = function(){
		...
		tabContTitle.text(updateTabName);
...
function lf_createDashboardTabContent(dashboardTemplateName){
	...
	// tab 생성 부분
	var $createLI = $('<li onclick="javascript: lf_tabEventClickLoadDashboard('+createIdx+');" rel="dashboardTabMgmt_'+createIdx+'" class="on '+activeClass+'"></ul>')
					...
					.append($('<a href="#" onclick="javascript: lf_openTabEditDialog(this);" class="tab_edit">edit</a>'));
	$UL.append($createLI);
...
function lf_removeTab(tabidx) {
	...
	lvar_dashboardData.splice(tabidx-1, 1);
	
	var activeIdx = tabidx-1;
	if(activeIdx > 0){
		lf_tabActive(activeIdx);
		lf_tabEventClickLoadDashboard(activeIdx);
	}
...
function lf_tabadd(){	
	...
	if (window.matchMedia('(min-width: 1920px)').matches) {
		...
		if ($lst_len == 0) {
			$('.tab_lst')
					.prepend(
							"<li onclick=\"javascript: lf_tabEventClickLoadDashboard("+count+");\" rel=\"dashboardTabMgmt_"
									+ count
									+ "\" class=\"open on\">\n"
									...
									+ "<a href=\"#\" onclick=\"javascript: lf_openTabEditDialog(this);\" class=\"tab_edit\">edit</a>\n"
									+ "</li>");
			$('.tab_container')
					.prepend(
			...
			$('.tab_lst')
					...
					.last()
					.after(
							"<li onclick=\"javascript: lf_tabEventClickLoadDashboard("+count+");\" ondblclick=\"javascript: lf_openDashboardFullWindow();\" rel=\"dashboardTabMgmt_"
									+ count
									+ "\" class=\"open on\">\n"
									+ "<a href=\"#\" class=\"tab_link\" title=\"\">Dashboard"
									+ count
