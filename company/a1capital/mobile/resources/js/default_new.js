document.addEventListener('DOMContentLoaded', function() {
  var button = document.querySelector('button[data-href]');
  var container = document.querySelector(button.getAttribute('data-href'));
...
$(document).ready(function() {
    ...
    $('#menuWrap .btn-menu-close').on('click', function() {
        $('#menuWrap').removeClass('menu-open');
    });

    $('.tab-slidemenu > ul > li').on('click', function() {
        $('.tab-slidemenu > ul > li').removeClass('on');
        $(this).addClass('on');
        $('.scroll-left-wrap .menuArea > li').removeClass('on');
        var index = $(this).index();
        $('.scroll-left-wrap .menuArea > li').eq(index).addClass('on');
    });
	...
	/**약관체크 팝업**/
          ...
          $('.terms-area').on('click', '.chk-all .check.all', function() {
              ...
              // 이 체크박스가 속한 .terms-area 내의 .list-wrap에 대해서만 dp-none 클래스 처리
              $(this).closest('.terms-area').find('.list-wrap').toggleClass('dp-none', isChecked);
          });
          ...
          function updateCheckAllState($termsArea) {
            // 체크박스 전체 개수
            var allCheckboxes = $termsArea.find('.level2 input[type="checkbox"]').length;
            // 체크된 체크박스 개수
            var checkedCheckboxes = $termsArea.find('.level2 input[type="checkbox"]:checked').length;
            // 전체 동의 체크박스 상태 업데이트
